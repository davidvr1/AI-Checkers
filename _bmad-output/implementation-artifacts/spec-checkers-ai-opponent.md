...
- **2026-07-22, human-initiated renegotiation (Israeli/international draughts ruleset):** The frozen "Always" bullet stating "All existing rules (mandatory captures, multi-jump chaining, kinging, draw-after-40, win-on-no-moves) are unchanged" is superseded: the *shared* rules engine (`rules.ts`/`gameReducer.ts`, consumed by both modes) changed under the renegotiation recorded in `spec-checkers-game.md` — flying kings, the man-may-only-capture-a-king-forward restriction, threefold-repetition and insufficient-material draws, and the no-capture draw threshold raised 40 → 50 half-moves. The AI plugs into the same engine unchanged, so it inherits all of these automatically. Two AI-specific consequences are noted but deliberately NOT changed here: (1) the material evaluation still weights a king at 1.5 men, which under-values a flying king but stays within the eval-philosophy `Ask First` clause (a tuning question, not a defect); (2) flying kings raise the branching factor, so the fixed `DIFFICULTY_DEPTH` values search fewer effective plies within the unchanged `SEARCH_DEADLINE_MS` cap — latency stays bounded, strength was not re-benchmarked. Both are logged in `deferred-work.md`.
