...
- **2026-07-23, human-initiated feature (online multiplayer over LAN):** The frozen "Never" bullet "no online/networked multiplayer ... now" is superseded -- the user asked to play two-player over their private WiFi. Added a Node server (`server/index.ts`, express + ws) that serves the built SPA and hosts an authoritative WebSocket match via a pure `GameRoom` (`src/net/gameRoom.ts`): first device to connect is Red, second Black, rest spectate; the server holds the real `GameState`, validates every move through the existing `gameReducer` PLAY_MOVE seam (exactly the extensibility seam this spec's Design Notes anticipated), and broadcasts state to all clients. Client: a "Play Online" setup option → `OnlineGameScreen` + `useOnlineGame` hook (renders server state, resolves clicks to moves, auto-reconnects). Run with `npm run serve`. Known limitations logged in `deferred-work.md` (spectator auto-promotion, color preservation across a simultaneous double-drop, rate limiting). No authentication -- it trusts everyone on the LAN, appropriate for a private home network only.
